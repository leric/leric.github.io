...
I tried this skill after a concrete task: replacing demo authentication with real authentication.

The reflection did not produce a grand architecture proposal. It found smaller issues that were easier to miss. The auth entry point was not expressed as a single visible convention. Legacy and new routes handled authentication through different paths. The test setup hid the real login/session contract, so passing tests did not clearly explain how authenticated and anonymous clients should be created.

The suggested improvement was correspondingly small: introduce explicit `login_client()` and `anonymous_client()` test helpers, and add a short note near the authorization module to make the expected auth entry point visible.

That is the scale this skill is meant to operate at. It catches the missing connection, hidden convention, or weak contract that would normally be too small for an architecture review, but large enough to become an omission risk in the next similar change.
