...
Working from that idea, I wrote a skill around it. The full text lives on GitHub: [post-change-design-reflection/SKILL.md](https://github.com/leric/leric.github.io/blob/main/skills/post-change-design-reflection/SKILL.md). It has the agent check three kinds of problems.
...
**Boundary Bypass.** To finish the task, did the agent route around a boundary that was supposed to hold? Business logic dropped into a controller, a use case reaching straight for an external SDK, adapter details leaking into an inner layer, a block of existing logic copied instead of called through its proper boundary. The tests can still pass while a hole gets dug in the architecture.
...
That is the boundary of post-change reflection. The agent stays out of the architect's chair. After each change it lays out the context path it just walked and recommends the smallest safe response. It applies a how-level repair only when explicitly asked, and raises genuine design decisions — ownership, boundaries, new concepts — for a person to settle.
